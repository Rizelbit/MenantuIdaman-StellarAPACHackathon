...
- **Keputusan final (2026-07-16, cross-ref `sprint/sprint-3-integration.md` S3-08):** ternyata `ReceiveScreen` ini **bukan** screen "Rp X masuk" dinamis yang sempat diasumsikan issue ini masih akan dibangun — itu screen yang berbeda dan tidak pernah dibuat. Tim memutuskan **pakai `ReceiveScreen` yang ada ini apa adanya** (frontend sudah di-lock), dan menyesuaikan demo script + resolve recipient di backend supaya nama penerima yang diketik saat demo ("Rani Putri") konsisten dengan nama yang di-hardcode di screen ini. Lihat `sprint/sprint-4-polish-demo.md` S4-05 untuk script yang sudah direvisi.
